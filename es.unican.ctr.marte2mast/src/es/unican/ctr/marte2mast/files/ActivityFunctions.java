 ...
 * from the model's activity diagrams and generate the MAST's text for  operations and 
 * transactions.
 * 
 * <p>Two of the methods are called from Acceleo's marte2mast.mtl template: extractActivityData
 * and getOperationsTransactionsText
 */
public class ActivityFunctions {
	...
	/** Used to store a map of internal events to last event operation (step). 
	 * we need this for Mast.java so we can update the model with the data results from MAST
	*/
	static HashMap<String,String> mIntEvent2OperationMap = new HashMap<String, String>();
	/** Used to store a list of lists of root-last children-last children... elements we need to update the UML model */
	//static Vector<Element> mTransactionLastChildrenList = new Vector<Element>();
	...
	/** Used to store a list of root-last children-last children... elements we need to update the UML model */
	static Vector<fatherChildrenStruct> mFatherChildrenList = new Vector<fatherChildrenStruct>();
	
	//////////////////////////////
	/**
	 * Extracts the context data from the "SaAnalysisContext" stereotyped element.
	 * If the invoke option, for the mast tool, is set to false, Mast.mOpenGmast will 
	 * be set to false, in any other case it will be set to true.
	 * 
	 * <p>we also check for a mode text here.
	 *  
	 * @param  theContext    "SaAnalysisContext" stereotyped element
	 */
	public static void extractContextInfo(Element theContext){
		...
		if(HelperFunctions.getStereotypeProperty(theContext, "SaAnalysisContext", "context")!=null){
			//example: (tool=mast,options=(invoke=true,recoverResults=true,overwriteResults=false,overwriteOutputModel=true,mode='The_mode:text._-'))
			...
			//mast modeID ->String 'alphanumerics_and_numbers_points_underscore_and_score', no commas o spaces?
			if(    HelperFunctions.parseContext(inputText, "mast", "modeID")!=null
				&& !HelperFunctions.parseContext(inputText, "mast", "modeID").equals("MALFORMED INPUT")
				&& !HelperFunctions.parseContext(inputText, "mast", "modeID").equals("")
				&& !HelperFunctions.parseContext(inputText, "mast", "modeID").equals("''")				
			){
	...
	/**
	 * Creates a new name for the internal event from the old one. 
	 * Right now it outputs the input name, so maybe this isn't needed.
	 *  
	 * @param  name    the original name
	 */
	public static String createInternalEventText(String name){
		//[TODO] check if I'll need to expand this in the future or I can get rid of it
		String theText =""+name;
		return theText;
	}//getInternalEvent
	
	/**
	 * Creates a MAST simple operation with execution times set to 0.0 called DummyOperation.
	 * 
	...
	/**
	 * Recursive method that checks if a given element ("SaStep" stereotyped), or any of its 
	 * 'children' (in the subUsage property or an associated Activity) has a value set for the 
	 * concuRes property of the stereotype.
	 *  
	 * @param  root            the initial element
	 * 
	 * @param  checkThisOne    true if you want to check the root element or only its 'children'
	 * 
	 * @return                 <code>true</code> if an element with concurRes was found, <code>false</code> if not
	 */
		...
		if( HelperFunctions.hasStereotype(root, "SaStep") ){
			...
			if(simpleOperations.size()!=0){//subUsage has data				
					for(Operation anOperation:simpleOperations){
						if(treeHasConcurRes(anOperation,true))return true;
					}	
			}//if subUsage data
			//activities--------------------------
			if(root instanceof Operation){//code taken, modified, from getActivityFromOperation
				...
				if(initialNode!=null){
					...
					for (NamedElement anElement : pathElements) {
						if(treeHasConcurRes(anElement,true))return true;
					}//end for
				}
			}//end if operation
	 ...
	 * Recursive method that checks if a given element ("SaStep" stereotyped), or any of its 
	 * 'children' (in the subUsage property or an associated Activity) has a value set for the 
	 * execTime property of the stereotype.
	 *  
	 * @param  root            the initial element
	 * 
	 * @param  checkThisOne    true if you want to check the root element or only its 'children'
	 * 
	 * @return                 <code>true</code> if an element with execTime was found, <code>false</code> if not
	 */
	public static boolean treeHasExecTime(NamedElement root, boolean checkThisOne) {
		if( HelperFunctions.hasStereotype(root, "SaStep") ){
			...
			if(simpleOperations.size()!=0){//subUsage has data				
					for(Operation anOperation:simpleOperations){
						if(treeHasExecTime(anOperation,true))return true;
					}	
			}//if subUsage data
			//activities--------------------------
			if(root instanceof Operation){//code taken, modified, from getActivityFromOperation
				...
				if(initialNode!=null){
					...
					pathElements = extractActivityElements(initialNode);
					for (NamedElement anElement : pathElements) {
						if(treeHasExecTime(anElement,true))return true;
					}//end for
				}		
			}//end if operation
	...
	/**
	 * Adds all the data left to complete the current event handler (MAST) stored in 'mEventHandler' and 
	 * adds it to the 'mEventHandlerList' list of event handlers. A new event handler structure is then 
	 * created (in 'mEventHandler').
	 * 
	 * <p>When a handler is closed, a new composite operation (MAST) is created, containing all the operations 
	 * stored in the list 'mEventHandlerOperationList' (the list is then cleared for later use). 
	 * This list is filled by 'extractStepData'.
	 * 
	 * <p>Every time the current thread changes, or we reach the end of the transaction, we have to call 
	 * this method to complete and store the opened handler.
	 */
	public static void closeEventHandler(){
		//add in event to list
		//String tempEventName = mTransactionName+"__Internal_Event_" + (++mInternalEventCounter);
		...
		mNextInputEvent=tempEventName;//next one will start from the output of this one
		//create common operation if needed and add it
		if(mEventHandlerOperationList.size()>0){
			
			if(mEventHandlerOperationList.size()==1){//just one -> no need to create one
				...
				for(int i=0;i<mEventHandlerOperationList.size();i++){
					...
					if(i<mEventHandlerOperationList.size()-1) tempList += ", ";
				}
				...
				mCompositeOperationsText+=tempText; tempText="";
...
//	//used with 'simple' groups of ops/steps (no concurRes inside) to ignore composites of just one element
...
//	}
	...
	/**
	 * Given an element ('SaStep' stereotyped), it returns the corresponding text for a simple operation (MAST). 
	 * It also stores the name of the operation in a list called 'mSimpleOperationsNames' in order to not repeating 
	 * operations in the MAST files when they are used more than once.
	 * 
	 * @param      theElement   the UML element
	 * 
	...
	public static String getSimpleOperationText(NamedElement theElement) {
		...
		if (HelperFunctions.getStereotypeProperty(theElement, "SaStep","execTime")!=null){
			...
			if ( HelperFunctions.parseNFP_Duration(HelperFunctions.getStereotypeProperty(theElement, "SaStep","execTime").toString(),"best").equals("MALFORMED INPUT") 
					&& HelperFunctions.parseNFP_Duration(HelperFunctions.getStereotypeProperty(theElement, "SaStep","execTime").toString(),"worst").equals("MALFORMED INPUT") ){
	...
	/**
	 * Given an element ('SaStep stereotyped) and a vector containing the names of the operations it contains, 
	 * it returns the corresponding text for a composite operation (MAST).
	 * 
	 * @param      theElement         the UML element
	 * 
	 * @param      simpleOperations   a vector containing the names of the operations it contains
	 * 
	...
	/**
	 * Given an element ('SaStep stereotyped) and a vector containing the names of the operations it contains, 
	 * it returns the corresponding text for an enclosing operation (MAST).
	 * 
	 * @param      theElement         the UML element
	 * 
	 * @param      simpleOperations   a vector containing the names of the operations it contains
	...
	public static String getEnclosingOperationText(NamedElement theElement, Vector<String> simpleOperations) {
		...
		if(simpleOperations!=null){
			...
			//old:
//			for(int i=0;i<simpleOperations.size();i++){
//				outputText += HelperFunctions.getElementLongName(simpleOperations.get(i));//simpleOperations.get(i).getName();
//				if(i<simpleOperations.size()-1) outputText += ", ";
//			}
			for(int i=0;i<simpleOperations.size();i++){
				outputText += simpleOperations.get(i);
	...
	/**
	 * Given an element ('GaWorkloadEvent' stereotyped) containing valid Arrival Pattern data (MARTE), it 
	 * returns the text for an external event (MAST)
	 * 
	 * @param   elt  an element ('GaWorkloadEvent' stereotyped)
	 * 
	...
	public static String getExternalEventText(Element elt) {
		...
		if(HelperFunctions.getArrivalPatternType(elt)==null){//----------------------------- ERROR ----------------
			Log.println("Warning: no valid pattern found in GaWorkloadEvent " + Log.getFileNameAndLineNumber(Thread.currentThread().getStackTrace()[1]));//changed from error to warning
		}
		//----------------------------- PERIODIC ----------------
	...
	/**
	 * Returns the text for all the internal events stored in the list 'internalEventList'. The parameter 'theConstraint' is 
	 * used to created the global timing requirements for the last event and the parameter 'gaWorkloadEvent' is used to 
	 * get the name of the external event.
	 * 
	 * <p>Called from 'extractActivityData'.
	 * 
	 * @param   internalEventList  a list of all the internal events
	 * @param   theConstraint      a constraint element ('GaLatencyObs' stereotyped) 
	 *                             containing the global timing requirements' data
	 * @param   gaWorkloadEvent    an element ('GaWorkloadEvent' stereotyped) 
	 *                             corresponding to the external event
	...
	public static String getInternalEventsText(Vector<String> internalEventList, Constraint theConstraint, NamedElement gaWorkloadEvent){
		...
		for (String theEvent:internalEventList){
			...
			//-------------------------------------------
			//check how many timing requirements we have. if we have more than one, we will use a composite type
			//numTiming will store how many requirements we have and actualTiming will store which one we are
			//printing (so we put a comma or not at its end)			
			int numTiming =0;
			int actualTiming=0;
			//[TODO] local timing reqs' code commented out till I know how we are going to use it after what we changed 		
//			if( HelperFunctions.getStereotypeProperty(stepElement,"SaStep","deadline") != null) numTiming+=1;
			if(index==numEvents && theConstraint!=null){//LAST STEP with global reqs
				...
				if(theMissRatio!="MALFORMED INPUT" && theLatency!="MALFORMED INPUT"){
					numTiming+=1;
				}else if(theLatency!="MALFORMED INPUT") numTiming+=1;
				if(theJitter!="MALFORMED INPUT") numTiming+=1;
			}
			...
			//[TODO] rework the code below to reuse it now so we have local timing reqs again
//			//-------------------------start rqs----------------------------------------
//			
//			//local:
//			if( HelperFunctions.getStereotypeProperty(stepElement,"SaStep","deadline") != null){	
//				//intEvtsText +=",\n         Timing_Requirements => (";
//				actualTiming+=1;
//				if(numTiming>1) intEvtsText +="\n               (";
//				intEvtsText +="\n                Type              => Hard_Local_Deadline,";
//				intEvtsText +="\n                Deadline          => " + HelperFunctions.parseNFP_Duration(HelperFunctions.getStereotypeProperty(stepElement,"SaStep","deadline").toString(), "value");
//				if(numTiming>1) intEvtsText +="\n               )";
...
//	    	}//end if SaStep
			...
			if(index==numEvents && theConstraint!=null){//LAST STEP with global reqs
				...
				if(theMissRatio!="MALFORMED INPUT" && theLatency!="MALFORMED INPUT"){
					...
					intEvtsText += "\n                Deadline          => " + theLatency;
					intEvtsText +=",\n                Referenced_Event  => " + ((NamedElement) gaWorkloadEvent).getName();//old: worng:+ inEvtName;
					//intEvtsText +=" )";
	...
	/**
	 * Given an element (instance of Operation) this method returns the Activity element 
	 * associated to it, if exists.
	 *
	 * @param  theElement      UML Element (Operation)
	 * 
	...
	/**
	 * Called from extractStepData when it reaches an operation (SaStep) with NO subUsages with concurRes or inner activities with concurRes. 
	 * This method calls itself recursively till there are no more operations to add. Simple operations are added to the mSimpleOperationsText  
	 * static member variable, composite operations to mCompositeOperationsText and enclosing operations to mEnclosingOperationsText.
	 *
	 * @param  rootElement      UML Element ("SaStep" stereotyped)
	 */	
	public static void generateOperations(NamedElement rootElement) {
		if( HelperFunctions.hasStereotype(rootElement, "SaStep") ){
			
			//it HAS to be a simple/enclosing -> CAN'T have subUsages with concurRes or inner activities with concurRes
			
			boolean checkSubusages=true;
			if(HelperFunctions.getStereotypeProperty(rootElement, "SaStep", "execTime")!=null){//simple or enclosing
				if(treeHasExecTime(rootElement, false)){//create enclosing
					
					Vector<String> enclosingOperationsList = new Vector<String>();//to create the enclosing text
					//////////////////////////////////////////////////////////////////////
					//check children					
					//activities--------------------------
								...
								if(theMethods.get(i) instanceof Activity){
									...
									for(int j=0;j<activityElements.size();j++){
										if(activityElements.get(j) instanceof InitialNode){
											...
											//code modified from extractActivityData
					...
					if(simpleOperations.size()!=0 && checkSubusages){//subUsage has data & there was no activity data (design decision)				
							for(Operation anOperation:simpleOperations){
			...
			}else{//no execTime: composite or 'nothing' (no execT no concurRes)
				if(treeHasExecTime(rootElement, false)){//composite
					Vector<String> compositeOperationsList = new Vector<String>();//to create the composite text
					//////////////////////////////////////////////////////////////////////
					//check children					
					//activities--------------------------
					if(rootElement instanceof Operation){
								...
								if(theMethods.get(i) instanceof Activity){
									...
									for(int j=0;j<activityElements.size();j++){
										if(activityElements.get(j) instanceof InitialNode){
											//check activity steps:
											InitialNode theInitialNode = (InitialNode) activityElements.get(j);	
											//code modified from extractActivitiData
											//activity elements extraction:
											Vector<NamedElement> pathElements= new Vector<NamedElement>();
											pathElements = extractActivityElements(theInitialNode);
											for (NamedElement anElement : pathElements) {
					...
					//create composite://TODO it said enclosing before, check if it was an error or there's something to it.
	...
	/**
	 * Extracts the relevant data from a given element ("SaStep" stereotyped). This method is called from 
	 * each of the elements on the list created by extractActivityElements and calls itself recursively for each of 
	 * its valid ("SaStep" stereotyped) children elements (from an Activity if it has one associated or from its subUsage list).
	 * 
	 * <p> The parentThread parameter is used to keep track of the caller element's concuRes, which is important to generate 
	 * the event handlers (MAST).
	 *
	 * @param  theElement       UML Element (should be an Activity)
	 * 
	 * @param  parentThread     concuRes of the parent element.
	 * 
	 * @return                  a vector containing the relevant elements of the Activity diagram in order
	 */
	public static boolean extractStepData(NamedElement theElement, String parentThread) {		
		if( HelperFunctions.hasStereotype(theElement, "SaStep") ){
			...
			if (HelperFunctions.getStereotypeProperty(theElement, "SaStep", "concurRes")!=null){
				...
				if(!HelperFunctions.getStereotypeReferenceName(theElement, "SaStep", "concurRes").toString().equals(mActualThread)){
					...
					if (!mEventHandler.mInputEvent.equals("")){
						//there's an opened event -> close old, add to list and create empty one
						closeEventHandler();					
					}//end opened event	------				
				...
				if( !parentThread.equals(mActualThread)){//actual and parent are different
			...
			if(HelperFunctions.getStereotypeProperty(theElement, "SaStep", "execTime")!=null){
				//it HAS to be a simple/enclosing -> CAN'T have subUsages or inner activities with concurRes
			...
			}else if(initialNode != null){//no execTime BUT an associated ***Activity***
				//activity elements extraction:
				Vector<NamedElement> pathElements= new Vector<NamedElement>();
				pathElements = extractActivityElements(initialNode);			
				...
				for (NamedElement anElement : pathElements) {
					...
					extractStepData(anElement, tempActualThread);//TODO: check if mActualThread is the correct one
				}//end for	
				...
				if(simpleOperations.size()!=0){//subUsage has data
					...
					for(NamedElement nextStep:simpleOperations){
						...
						if(!extractStepData(nextStep, tempActualThread))return false;
					}
					if(lastChildren!=null){
						fatherChildrenStruct aFatherChildrenStruct=new fatherChildrenStruct();
						aFatherChildrenStruct.mElement=lastChildren;
	...
	/**
	 * Extracts an vector containing the relevant elements ("SaStep" stereotyped) of the Activity diagram in order.
	 * Given an InitailNode ("GaWorkloadEvent" stereotyped) it follows the diagram elements until it reaches the end.
	 * i.e. ControlFlow->ActivityNode->ControlFlow->ActivityNode->...->ActivityNode->-| 
	 *
	 * @param  anElement       UML Element (should be an Activity)
	 * 
	 * @return                 a vector containing the relevant elements of the Activity diagram in order
	 */
	...
	/**
	 * Static method called from Acceleo's marte2mast.mtl template for every Activity with the stereotype "SaEndtoEndFlow" 
	 * (also, the Activity has to be owned by an element with the stereotype "SaAnalysisContext").
	 * 
	 * <p>Uses other methods from this class, like extractActivityElements, extractStepData, 
	 * closeEventHandler, getExternalEventText ... to generate the text for this transaction.
	 *
	 * @param  elt      should be a UML Activity with the restrictions above described
	 */
...
//				Vector<fatherChildrenStruct> fc = mTransactionFatherChildrensLists.get(i);
...
//			}
			...
			tempText+="    Name            => " + activityName + ",\n";//transaction's name =  activity's name
	        ...
	        Log.debugPrintln("finished storing text from a transaction");Log.debugPrintln("");
			//CLEAN UP
	...
	/**
	 * Static method called from Acceleo's marte2mast.mtl template, after extracting the data from all the activities, to return the 
	 * text describing the MAST operations and transactions.
	 *
	 * @param  elt             UML Element from which the method is called in Acceleo
	 * 
	...
	public static Object getOperationsTransactionsText(Element elt) {
		...
		output+= "-- Operations\n\n"
				...
				+ mTransactionsText;		
		//CLEAN UP (plug-in)/////////////////////
		//there's some kind of data persistence between different executions of the plug-in!!
